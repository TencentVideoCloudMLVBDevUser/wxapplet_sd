	...
	var listeners = {
		...
		"onBigGroupMsgNotify": function (msg) {
			webimhandler.onBigGroupMsgNotify(msg, function (msgs) {
                ...
                //收到白板数据
                console.log("----------------> RTCRoom callback --> 收到白板数据")
...
function onSketchpadData(data){
    console.log("-------------> RTCROOM OnSketchPadData Called()");
 ...
 *   pushers: 退房人员列表
 */
function mergePushers(completeCallback) {
	// CGI拉取房间成员列表开始时间戳
...
function sendC2CCustomMsg(object) {
	...
	webimhandler.sendC2CCustomMsg(object.toUserID?object.toUserID:roomInfo.roomCreator, msg, function (ret) {
		if (ret && ret.errCode) {
			console.error('发送自定义C2C消息失败:', JSON.stringify(ret));
